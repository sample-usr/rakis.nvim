...
local function get_hl_group(hl)
  local group = "RakisDev" .. vim.inspect(hl):gsub("%W+", "_")
...
local function get_group(buf)
  ...
  if not fname:find("lua/rakis/extensions/") then
    return
  end
...
vim.api.nvim_create_autocmd("BufWritePost", {
  group = vim.api.nvim_create_augroup("rakis_dev", { clear = true }),
  pattern = "*/lua/rakis/**.lua",
  callback = vim.schedule_wrap(function(ev)
    local opts = require("rakis.config").options
    for k in pairs(package.loaded) do
      if k:find("^rakis") then
        package.loaded[k] = nil
      end
    end
    require("rakis").setup(opts)
...
return {
  {
    ...
    opts = function(_, opts)
      ...
      opts.highlighters.rakis_colors = {
        pattern = {
          "%f[%w]()t%.[%w_%.]+()%f[%W]",
        },
