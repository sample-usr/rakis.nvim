...
function M.get(options, p)
  ...
  local highlights = {
    ...
    MiniDiffOverChange = { bg = util.blend(p.bg_solid, p.dusk, 0.8) },
    MiniDiffOverContext = { bg = util.blend(p.bg_solid, p.spice, 0.8) },
    MiniDiffOverDelete = { bg = util.blend(p.bg_solid, p.blood, 0.8) },
    MiniDiffSignAdd = { fg = p.oasis },
    MiniDiffSignChange = { fg = p.spice },
    MiniDiffSignDelete = { fg = p.blood },

    MiniFilesBorder = { fg = p.muted },
    MiniFilesBorderModified = { fg = p.dusk },
    ...
    MiniStatuslineModeReplace = { fg = p.bg_solid, bg = p.blood, bold = true },
    MiniStatuslineModeVisual = { fg = p.bg_solid, bg = p.bloom, bold = true },

    MiniSurround = { link = "IncSearch" },

    MiniTablineCurrent = { fg = p.text, bg = p.muted, bold = true },
    MiniTablineFill = { link = "TabLineFill" },
    MiniTablineHidden = { fg = p.muted, bg = p.bg },
    MiniTablineModifiedCurrent = { fg = p.muted, bg = p.text, bold = true },
    MiniTablineModifiedHidden = { fg = p.bg_solid, bg = p.muted },
    MiniTablineModifiedVisible = { fg = p.muted, bg = p.text },
    MiniTablineTabpagesection = { link = "Search" },
    MiniTablineVisible = { fg = p.text, bg = p.muted },
